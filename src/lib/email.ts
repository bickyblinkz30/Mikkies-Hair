...
import { generateToken } from "./booking-token"
...
type SendBookingEmailParams = {
  ...
  time: string
  stylistName: string
  type: "confirmation" | "pending" | "declined" | "reminder" | "stylist_pending"
...
export async function sendBookingEmail({
  ...
  if (type === "stylist_pending" && appointmentId) {
    const token = await generateToken(appointmentId)
    const confirmUrl = `${baseUrl}/api/booking/confirm?id=${appointmentId}&token=${token}`
    const declineUrl = `${baseUrl}/api/booking/decline?id=${appointmentId}&token=${token}`
      ...
      <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background: #0a0a0a; margin: 0; padding: 0;">
        <div style="max-width: 600px; margin: 0 auto; padding: 24px;">
          <div style="background: linear-gradient(135deg, #1a1a1a, #0d0d0d); padding: 32px; border-radius: 16px 16px 0 0; text-align: center; border-bottom: 2px solid #C9A84C;">
            ...
            <p style="color: rgba(255,255,255,0.6); margin-top: 4px; font-size: 14px;">New Booking Request</p>
          </div>
          <div style="background: #111; padding: 32px; border-radius: 0 0 16px 16px;">
            <h2 style="color: #fff; font-size: 20px; margin: 0;">Hello ${stylistName},</h2>
            <p style="color: #9ca3af; line-height: 1.6; margin-top: 12px;">You have a new booking request.</p>
            ...
            <table style="width: 100%; margin-top: 24px; border-collapse: collapse;">
              <tr>
                <td style="width: 50%; padding-right: 8px;">
                  <a href="${confirmUrl}" style="display: block; text-align: center; background: #C9A84C; color: #000; text-decoration: none; padding: 14px 20px; border-radius: 8px; font-weight: 600; font-size: 14px;">✓ Confirm Booking</a>
                </td>
                <td style="width: 50%; padding-left: 8px;">
                  <a href="${declineUrl}" style="display: block; text-align: center; background: transparent; color: #ef4444; text-decoration: none; padding: 14px 20px; border-radius: 8px; font-weight: 600; font-size: 14px; border: 1px solid #ef4444;">✗ Decline Booking</a>
                </td>
              </tr>
            </table>
            <p style="color: #6b7280; font-size: 12px; text-align: center; margin-top: 16px;">
              You can also manage this booking from your dashboard.
            </p>
          </div>
